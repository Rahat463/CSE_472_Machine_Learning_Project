% ============================================================
% NeurIPS Format Paper: Knowledge Distillation of DNABERT-2
...
\author{
Anup Halder Joy\thanks{Equal contribution.} \\
Student ID: 2005005\\
Dept.\ of CSE, BUET\\
Dhaka, Bangladesh\\
\texttt{2005005@ugrad.cse.buet.ac.bd}
\And
Sheikh Rahat Mahmud\footnotemark[1] \\
Student ID: 2005048\\
Dept.\ of CSE, BUET\\
Dhaka, Bangladesh\\
\texttt{2005048@ugrad.cse.buet.ac.bd}
\And
Md.\ Roqunuzzaman Sojib \\
Supervisor, Lecturer\\
Dept.\ of CSE, BUET\\
Dhaka, Bangladesh\\
\texttt{sojib@teacher.cse.buet.ac.bd}
}
...
\node[block, fill=gray!10, above right=0.5cm and 0.5cm of hardloss] (gt) {Ground Truth $y$};
...
% Hard loss connections: Student and Ground Truth -> Cross-Entropy
\draw[arrow] (student.north east) -- (hardloss.south west);
\draw[arrow] (gt) -- (hardloss);

% Losses -> total
\draw[arrow] (softloss) -- (total);
\draw[arrow] (hardloss.north) |- (total.east);

% Backprop
\draw[dasharrow, blue!60] (total.south west) -- ++(-0.5,0) coordinate (bpcorner) |- ([yshift=0.3cm]student.north) -- (student.north);
\node[font=\scriptsize, text=blue!60, left] at ([yshift=-1.5cm]bpcorner) {Backprop};
\end{tikzpicture}
...
\textbf{Architecture} & $\boldsymbol{\alpha}$ & Acc & F1 & Acc & F1 & Acc & F1 \\
\midrule
\multirow{2}{*}{Hybrid-CNN-3L}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{student_distillation_result.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{pure_architecture_results.png}
...
Soft labels at $T\!=\!5.0$ reveal inter-class similarities---e.g., Splice~A samples show nonzero probability for other splice classes---providing ``dark knowledge'' that hard one-hot labels cannot convey~\cite{hinton2015distilling}.
...
\textbf{Why KD helps the Hybrid but hurts the 6-Layer.}\quad Without augmentation, KD degrades the 6-Layer by 1.4\% while boosting the Hybrid by 3.8\%. Three factors explain this: (1)~\emph{teacher overconfidence}---on the small 13.6K dataset, soft-label entropy is only 1.515\,bits, adding noise rather than signal~\cite{hinton2015distilling, li2023curriculum}; (2)~\emph{capacity gap}---the 6-Layer (46.7\,M) is close enough to the teacher (117\,M) to overfit noisy soft labels, while the Hybrid's smaller capacity acts as implicit regularization~\cite{mirzadeh2020improved}; (3)~\emph{inductive bias}---the Hybrid's CNN kernel (size~9) constrains the student toward biologically relevant motifs (TATA box $\sim$10\,bp).

\textbf{Data augmentation as teacher regularization.}\quad With augmentation, teacher accuracy drops from 96.6\% to 91.8\%, and KD becomes beneficial even for the 6-Layer ($+0.3\%$). Augmentation prevents overfitting and produces softer, more informative distributions~\cite{tang2020understanding}. We recommend \emph{always augmenting before distillation} on datasets with $<$20K samples.

\textbf{Cross-species generalization.}\quad Both students outperform the teacher on mouse promoters (82.61\% and 82.24\% vs.\ 79.66\%), suggesting distillation acts as a regularizer against species-specific overfitting. Notably, the pure transformer---with no pretrained weights---nearly matches the DNABERT-2-initialized Hybrid on mouse data, implying the generalization benefit comes from distillation itself. Near-random plant performance confirms mammalian features do not transfer across kingdoms.

\textbf{Practical deployment.}\quad The Hybrid's $2.7\times$ CPU speedup and 105\,MB size make it deployable on standard laptops, processing 1,000 sequences in $\sim$73\,s vs.\ $\sim$197\,s for the teacher.
...
\bibitem[Ji et~al.(2021)]{ji2021dnabert}
Y.~Ji, Z.~Zhou, H.~Liu, and R.~V.~Davuluri.
{DNABERT}: pre-trained bidirectional encoder representations from transformers model for {DNA}-language in genome.
\emph{Bioinformatics}, 37(15):2112--2120, 2021.

\bibitem[Zhou et~al.(2023)]{zhou2023dnabert2}
Z.~Zhou, Y.~Ji, W.~Li, P.~Dutta, R.~Davuluri, and H.~Liu.
{DNABERT-2}: Efficient foundation model and benchmark for multi-species genome.
\emph{arXiv preprint arXiv:2306.15006}, 2023.

\bibitem[Dalla-Torre et~al.(2023)]{dallatorre2023nucleotide}
...
In \emph{Findings of EMNLP}, pages 4163--4174, 2020.
...
In \emph{AAAI}, pages 5191--5198, 2020.

\bibitem[Li et~al.(2023)]{li2023curriculum}
C.~Li, H.~Yang, J.~Zhang, et~al.
Curriculum temperature for knowledge distillation.
In \emph{AAAI}, 2023.

\bibitem[Tang et~al.(2020)]{tang2020understanding}
J.~Tang, R.~Shivanna, Z.~Zhao, D.~Lin, A.~Singh, E.~H.~Chi, and S.~Jain.
Understanding and improving knowledge distillation.
\emph{arXiv preprint arXiv:2002.03532}, 2020.

\bibitem[Oubounyt et~al.(2019)]{oubounyt2019deepromoter}
M.~Oubounyt, Z.~Louadi, H.~Tayara, and K.~T.~Chong.
{DeePromoter}: Robust promoter predictor using deep learning.
\emph{Frontiers in Genetics}, 10:286, 2019.

\bibitem[Zhang~Lab(2024)]{zhangtaolab2024plant}
{Zhang Lab}.
...
In \emph{NeurIPS}, pages 4694--4703, 2019.

\bibitem[InstaDeep(2023)]{instadeep2023dataset}
{InstaDeep}.
